...
| Notificações push (FCM) — ponta a ponta | **Cliente**: `src/pwa/notifications.ts` (`requestAndRegisterPushToken` + `listenForForegroundPush`), cache do token em `src/pwa/pushTokenCache.ts`, chamado no `src/layout/AppShell.tsx`. **Service worker**: gerado em build pelo plugin `generateFirebaseMessagingSW` no `vite.config.ts` → `public/firebase-messaging-sw.js` (não commitado, está no `.gitignore`). **Servidor**: `functions/src/push.ts` (`sendPushToUser`), agendadas em `functions/src/automation.ts` (`closeInvoicesDue` 00h, `generateRecurrences` 06h, `sendDueReminders` 08h, `sendDailyLogReminder` 20h) e `functions/src/budgetAlerts.ts` (10h). Tokens em `users/{uid}/fcmTokens/{token}`. ⚠️ **Duas armadilhas já pagas caro** (ver `docs/history/2026-07.md`): o SW do FCM precisa de **escopo próprio** (`/firebase-cloud-messaging-push-scope`) porque o VitePWA ocupa `/`, e `getRegistration()` casa por escopo — não por script; e o `link` do webpush tem que ser **URL absoluta HTTPS**. Testes: `src/pwa/notifications.test.ts`. Diagnóstico: `docs/RUNBOOK.md` |
