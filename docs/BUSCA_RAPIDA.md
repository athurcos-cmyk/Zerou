...
| Resumo do dia no Extrato + cor/sinal da linha | `transactionFlowByType` / `dayFlowTotals` (`src/finance/financeCalculations.ts`) classificam cada `TransactionType` em `spent`/`received`/`internal` (transferência e pagamento de fatura são internos e **não** entram no total). UI: `DayGroupSummary` em `TransactionsPage.tsx` + `.day-group-total*` (`global.css`). Regras visuais: `docs/design/DESIGN.md` |
