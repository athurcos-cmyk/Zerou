...
- **Estorno/reembolso/ajuste ABATEM o gasto da categoria** (`REFUND_TYPES`, desde 2026-08-06). Antes eram simplesmente ignorados, e a Vic dizia um gasto maior do que qualquer tela mostrava.
- ⚠️ **Compra parcelada no cartão conta UMA PARCELA POR MÊS** (desde 2026-08-06), não o valor cheio no mês da compra: R$ 588 em 4x pesa R$ 147 em cada um dos 4 meses. Vale pro gasto por categoria E pra TENDÊNCIA. Antes disso a Vic somava o valor cheio e respondia um número que **não existia em tela nenhuma** — a Análise mostra a parcela desde a ancoragem de 2026-08-05 e o Dashboard desde 2026-08-06.
  - O cronograma é reconstruído a partir da **própria transação** (`functions/src/shared/installmentSchedule.ts`, porta manual de `src/cards/installmentSchedule.ts`), **sem ler o ledger da fatura** — o ledger custaria leitura por mensagem. Os campos usados: `installments`, `invoiceId` (cujo id embute o `referenceMonth`), `installmentStart` (compra "já em andamento" não desloca a série) e `anticipatedInstallments` (parcela antecipada conta no mês em que se antecipou).
  - **Sincronia manual**: se a regra de ancoragem mudar em `src/finance/spendingAnalysis.ts`, este arquivo em `functions/src/shared/` tem que mudar junto (mesmo pacto de `shared/accountEffects.ts`).
  - Limite conhecido: a consulta pega 90 dias de transações, então a parcela de uma compra **mais antiga que isso** não é contada aqui — nas telas ela aparece (o ledger tem a parcela). Divergência residual documentada.
...
13. **Orçamento: não promete aviso automático, e a tela certa é a Análise** (2026-08-06). Duas correções no mesmo dia: (a) o app **deixou de ter notificação de orçamento** — o banner do Dashboard e a Cloud Function `sendBudgetAlerts` foram removidos porque contavam compra parcelada pelo valor cheio, e o prompt agora proíbe prometer que a pessoa "vai ser avisada"; (b) a regra 12 apontava "Metas/Orçamentos" como se existisse tela de Orçamentos — o limite por categoria vive no **menu de mais ações da Análise**. O prompt também descreve que o limite mede gasto **direto** na categoria (subcategoria não soma no pai, `[D9]`) e vale sempre o mês corrente.
