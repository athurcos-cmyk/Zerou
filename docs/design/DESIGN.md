...
- **NÃO bloquear pull-to-refresh via `overscroll-behavior`** (tentado e revertido em 2026-07-19): `overscroll-behavior-y: contain` em `html, body` **travou todo o scroll no celular** (interação com o `overflow-x: hidden` do body + o modelo de scroll do documento). Era pra ser inofensivo pro scroll, mas não foi no navegador mobile real. O gesto nativo de "puxar pra recarregar" fica como está; o flash que ele causa já foi minimizado pelo cache do Dashboard.
