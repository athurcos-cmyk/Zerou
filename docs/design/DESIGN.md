...
- **Pull-to-refresh desabilitado** (`overscroll-behavior-y: contain` em `html, body`, `global.css`): o app é offline-first e sincroniza sozinho, então "puxar pra recarregar" (Chrome Android) só reiniciava o app à toa e reexibia o boot de 1-2s. Não reabilitar sem motivo — o reload pelo botão do navegador continua funcionando normalmente.
