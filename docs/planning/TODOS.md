...
## Concluído (recente)
- [x] **Projeção de meses futuros na Análise** (2026-07-12): dá pra avançar pra frente (até a última parcela/conta comprometida) e ver o que já está comprometido — parcelas de cartão + contas a pagar do mês, rotulado "Já comprometido" (não "Gasto"). Recorrências ficaram de fora de propósito (seriam estimativa). `committedByCategoryForMonth`/`lastCommittedMonth` em `spendingAnalysis.ts`. Verificado ao vivo. **Possível próximo passo**: camada "Previsto" projetando recorrências mês a mês, separada do comprometido real.
