...
Integracao oficial com a Meta Cloud API v25.0 para lancamento financeiro por mensagem de WhatsApp. O usuario vincula seu numero e pode: lancar despesa ("gastei 15 reais no mercado"), lancar receita ("recebi 200 de freela"), transferir entre contas ("transfere 100 do nubank pro itau"), lancar compra no cartao a vista ou parcelada ("gastei 300 no cartao em 3x") e criar categoria sob pedido explicito ("cria uma categoria chamada Pet"). Uma unica chamada DeepSeek classifica a intencao da mensagem e ja extrai os dados (`interpretMessage.ts`).

> **Atualizacao 2026-07-28 (decisao do dono):** (1) o WhatsApp **nao responde mais pergunta nenhuma** — geral ("como economizar?") ou sobre os dados ("quanto gastei"). Toda pergunta redireciona pra Vic do app (aba Assistente), que tem historico de conversa (`questionRedirectMessage()` em `messageFormat.ts`). O WhatsApp fica so pra LANCAR. O helper `answerFinancialQuestion.ts` foi deletado. (2) **Data retroativa**: se a mensagem citar quando aconteceu ("gastei 40 no mercado dia 20", "ontem", "20/07"), o `interpretMessage` extrai a data (`occurredOn`) e o lancamento vai com ela (ao meio-dia BRT), inclusive apos o "qual cartao/conta?" (guardada em `pendingAction.occurredOnISO`). Sem data citada, continua sendo hoje. Futuro/formato invalido cai pra hoje.

**Qualquer pedido claro que o bot nao executa por mensagem cai no intent `out_of_scope`** (unificado em 2026-07-25 — ver Historico): editar/excluir algo ja lancado ("exclui essa transacao", "corrige o valor"), criar/editar conta a pagar ou recorrencia ("cria uma conta pra pagar o aluguel todo mes", "cadastra a Netflix como conta fixa"), acao avancada de cartao, mudar uma conta bancaria cadastrada, mexer numa meta, etc. — sao reconhecidos e respondidos com orientacao pra tela certa do app (`suggestedScreen`, ver `interpretMessage.ts`), em vez de cair no "nao entendi" generico ou, pior, ser silenciosamente ignorado. Antes cada um desses casos exigia um intent novo, nomeado e documentado a parte (`unsupported_action`, `bill_management_action`, `advanced_card_action`) — hoje e um unico intent com uma tela sugerida, o que cobre automaticamente casos novos que ainda nao foram vistos, sem precisar de mais um intent por vez que alguem descobre algo que a Vic nao faz.

**`question` cobre so consulta pontual e autocontida** (refinado no mesmo dia, 2026-07-25, pedido do dono): "quanto gastei esse mes", "quanto tenho disponivel", "minhas contas venceram" — um dado especifico que responde sozinho. Pedido de ANALISE mais aberta/comparativa ("como estao minhas financas esse mes", "faz uma analise dos meus gastos", "como foi comparado ao mes passado", "qual a tendencia dos meus gastos") vira `out_of_scope` com `suggestedScreen: 'assistente'` em vez de `question` — motivo: esse tipo de pergunta normalmente puxa pergunta de acompanhamento ("e por categoria?", "e comparado a quando?"), e o WhatsApp responde cada mensagem isolada, sem guardar historico nenhum (diferente do app, que guarda as ultimas 10 mensagens) — o acompanhamento sempre sairia errado por aqui.
...
**Decisao financeira grande (emprestimo, financiamento, renegociar divida, cartao novo/anuidade) e QUALQUER pergunta de investimento** (desde 2026-07-18, criterio de investimento ampliado no mesmo dia; migrado pra `out_of_scope` + `suggestedScreen: 'assistente'` em 2026-07-25): o bot NAO tenta aconselhar — redireciona pro app, sempre com `suggestedScreen: 'assistente'` (nunca outra tela) e um tom de convite pra conversa, nao de recusa seca (ver `outOfScopeMessage()` em `messageFormat.ts`). Motivo: a Vic do app agora ajuda a pessoa a pensar em decisao de emprestimo/cartao (pergunta de volta usando os dados reais dela, ver `docs/ai/VIC.md` regra 10) e trata investimento com rigor redirecionando pra profissional (regra 11) — os dois exigem mais profundidade do que uma mensagem isolada de WhatsApp permite (sem historico de conversa, `answerFinancialQuestion.ts` nao recebe `history` nenhum, diferente do app que manda as ultimas mensagens a cada chamada). Decisao deliberada do dono: **nao levar esse comportamento pro WhatsApp** — aqui o foco continua sendo lancamento e pergunta rapida do dia a dia (`question` continua respondida direto). Rede de seguranca: `answerFinancialQuestion.ts` tambem ganhou instrucao pra redirecionar em vez de analisar, caso alguma mensagem escape da classificacao. Regra de nunca nomear banco/cartao/investimento especifico (mesmo se pedirem direto) tambem foi espelhada aqui. Diferente de `question` (consulta de dado tipo "quanto gastei") — so decisoes grandes/dificeis de desfazer e pergunta de investimento entram aqui.

**Importante**: a Vic do app (`financialAssistantChat`) e so leitura/conversa — nunca cria dados, mas tem historico e responde perguntas. O bot do WhatsApp e o oposto: **so lanca** (despesa/receita/transferencia/compra no cartao/categoria) e **nao responde pergunta nenhuma** (redireciona pro app, ver Atualizacao 2026-07-28 acima). Os dois se complementam: pergunta/analise/decisao no app, lancamento rapido no WhatsApp.

**Nome no WhatsApp**: Vic (a assistente de IA do Granativa)
...
| `functions/src/whatsapp/webhookHandler.ts` | Cloud Function `onRequest`. GET: verificacao de webhook. POST: le a mensagem, resolve `vincular XXXXXX` via regex (`processLinkCode`) ou chama `interpretMessage` (com a lista de contas ativas) e roteia por intencao: `create_category` → `createCategoryFromMessage`; `question` → `answerFinancialQuestion`; `expense`/`income`/`transfer` → resolve a conta via `accountResolution.ts` (nome citado → principal → pergunta) e chama `createTransactionFromMessage`; `out_of_scope` → loga (`whatsapp_out_of_scope`) e manda `outOfScopeMessage(suggestedScreen)`; `unclear` → mensagem de ajuda com exemplos. |
| `functions/src/whatsapp/interpretMessage.ts` | Uma chamada DeepSeek (JSON mode) classifica a mensagem em `expense`/`income`/`transfer`/`card_purchase`/`create_category`/`question`/`out_of_scope`/`unclear` e ja extrai valor, descricao, categoria (a mais especifica entre as existentes), dados de categoria nova, conta(s) citada(s) (`accountId`, ou `sourceAccountId`/`destinationAccountId` pra transfer) e, quando `out_of_scope`, a tela sugerida do app (`suggestedScreen`: `transacoes`/`categorias`/`contas`/`contas_a_pagar`/`contas_a_receber`/`cartoes`/`metas`/`analise`/`assistente`/`geral`). **O enum tem TRES pontos de sincronia** que o compilador nao cobre: o tipo `OutOfScopeScreen`, a lista `validScreens` (valor fora dela cai calado em `geral`) e a linha do schema no prompt. So o `switch` de `outOfScopeMessage` e barrado pelo `tsc` (sem `default`)., tudo numa chamada so. Recebe a lista de contas ativas do workspace (id: nome) junto com a de categorias. Substitui o antigo `extractExpense.ts` (so cobria despesa). |
...
| `functions/src/whatsapp/categoryPalette.ts` | Espelha `src/components/categoryIcons.tsx` e `src/theme/palette.ts` (Cloud Functions nao importa `src/` do app cliente — pacotes separados). **Sincronia garantida por teste**: `src/theme/categoryPaletteSync.test.ts` roda no `npm test` do app e falha se divergir (icones, cores e cor padrao, ordem inclusa — a ordem das cores importa porque a cor da categoria nova sai por rotacao no indice). Criado em 2026-07-29 depois de o espelho ficar preso em 36 icones/12 cores enquanto o app ja oferecia 122/24 — a Vic criava categoria com o conjunto antigo em silencio. **Mudar este arquivo exige deploy das functions** (`git push` nao reimplanta). |
| ~~`functions/src/whatsapp/answerFinancialQuestion.ts`~~ | **DELETADO em 2026-07-28** — o WhatsApp nao responde mais pergunta (redireciona pro app via `questionRedirectMessage()`). O intent `question` continua sendo detectado por `interpretMessage`, mas o handler redireciona em vez de responder. |
| `functions/src/whatsapp/messageFormat.ts` (2026-07-22, `outOfScopeMessage` adicionada 2026-07-25) | Templates das mensagens de saida do bot — confirmacoes de lancamento, prompts de escolha (cartao/conta/transferencia) e `outOfScopeMessage(screen)` (uma mensagem por `suggestedScreen`, `switch` sem `default` — o build (`tsc`) barra se um valor novo do enum `OutOfScopeScreen` ficar sem mensagem). Logica pura, sem Firestore, testada isoladamente (`messageFormat.test.ts`, mesmo padrao de `accountResolution.ts`). Fixa a convencao de emoji do bot inteiro: 💸 despesa · 💰 receita · 🔄 transferencia · 💳 cartao · 🏷️ categoria · 🏦 conta/banco · 🧭 fora do escopo · 📋 contas a pagar/receber · 🎯 metas · 📊 analise/orcamento. Nao mexer na paleta de emoji num arquivo isolado — sempre editar aqui pra nao divergir. |
...
| `functions/src/ai/deepseekClient.ts` | Cliente HTTP compartilhado para API DeepSeek. Usado por `interpretMessage.ts` (WhatsApp) e pela Vic do app (`financialAssistant.ts`). |
| `functions/src/ai/aiRateLimit.ts` | Rate limit de IA (60 msgs/dia por workspace, `workspaces/{id}/aiUsage/{data BRT}`) — usado pela Vic do app (`financialAssistant.ts`). Deixou de valer pro WhatsApp em 2026-07-28 (o WhatsApp nao chama mais DeepSeek pra responder pergunta — so pra classificar/extrair, que nao conta nesse limite). |
...
11. Roteamento por intent:
    ...
    - question: logga whatsapp_question_redirected → questionRedirectMessage() (redireciona pra Vic do app; nao responde mais, desde 2026-07-28)
    ...
    - out_of_scope: loga whatsapp_out_of_scope(suggestedScreen) → outOfScopeMessage(suggestedScreen) (sem chamar DeepSeek de novo) — pedido claro, so que de algo que o bot nao executa por mensagem
...
| Timeout DeepSeek (extracao) | `deepseekClient.ts:7` | 45s | Compartilhado com Vic |
...
**Se acontecer nesse projeto de novo**: nao e algo pra "consertar" preventivamente no codigo — e um risco inerente de depender da plataforma da Meta. Ver conversa sobre alternativa (PWA proprio de chat com a Vic, sem depender da Meta) em `docs/history/2026-07.md` — ainda nao decidido, ideia registrada pra o futuro.
...
### 2026-07-25 — Consolidacao: `advanced_card_action`/`unsupported_action`/`bill_management_action`/`advisory_decision` viram um unico intent `out_of_scope` + `suggestedScreen`

- **Pedido do dono**: o prompt do classificador (`interpretMessage.ts`) so crescia — cada vez que se descobria algo que a Vic nao faz via WhatsApp, a solucao era criar um intent novo com sua propria explicacao de varias linhas, seu proprio bloco de rota em `webhookHandler.ts` e sua propria mensagem fixa. Pior: qualquer pedido novo que ninguem tivesse pensado em enumerar caia em `unclear` — a mensagem generica "Nao entendi essa mensagem", enganosa quando o bot **entendeu perfeitamente** o pedido, so nao faz aquilo por mensagem. Ideia do dono: ensinar so a lista curta do que a Vic FAZ (whitelist) e tratar qualquer pedido claro fora dela como um redirecionamento generico — nunca "nao entendi".
- **Mudanca**: os 4 intents `advanced_card_action`/`unsupported_action`/`bill_management_action`/`advisory_decision` viram um so, `out_of_scope`, com um campo novo `suggestedScreen` (`transacoes`/`contas`/`contas_a_pagar`/`contas_a_receber`/`cartoes`/`metas`/`analise`/`assistente`/`geral`) que a IA preenche pra manter a precisao de apontar a tela certa — sem precisar de um intent nomeado por caso. `unclear` continua intocado e separado (mensagem sem pedido identificavel nenhum, diferente de `out_of_scope` que E um pedido claro). Decisao financeira grande/investimento sempre cai em `suggestedScreen: 'assistente'` (nunca `'geral'`) e mantem o tom de convite pra conversa, nao de recusa seca — reforcado por comentario no codigo (`outOfScopeMessage()` em `messageFormat.ts`) pra nao se perder se um caso futuro for mapeado ali por analogia errada.
- **`suggestedScreen` verificado contra as rotas reais** (`src/App.tsx`/`AppShell.tsx`): `'analise'` cobre orcamento (nao existe tela dedicada "Orcamentos" — vive em `SearchPage.tsx`/rota `/app/search`, nav "Analise"); `'contas'` e novo, cobre editar/excluir uma conta bancaria (os intents antigos nao cobriam isso explicitamente).
- **Refinamento no mesmo dia, pedido do dono**: "question" foi restringido a consulta pontual/autocontida ("quanto gastei", "quanto tenho disponivel") — pedido de ANALISE mais aberta/comparativa ("como estao minhas financas", "compara com o mes passado", "qual a tendencia dos meus gastos") passou a cair em `out_of_scope` com `suggestedScreen: 'assistente'` em vez de `question`. Motivo: pergunta desse tipo normalmente puxa acompanhamento ("e por categoria?", "e comparado a quando?"), e o WhatsApp responde cada mensagem isolada sem guardar as anteriores — diferente do app, que guarda as ultimas 10 mensagens (`AssistantPage.tsx`/`financialAssistant.ts`) e consegue continuar a conversa direito.
- **Observabilidade nova**: nenhum dos 4 blocos antigos logava nada — o bloco novo loga `whatsapp_out_of_scope` com `suggestedScreen`, mesmo padrao de `whatsapp_transaction_created`.
- **Bug de logging encontrado e corrigido no mesmo dia** (achado investigando a Vic nao responder — ver entrada abaixo sobre a migracao `deepseek-chat` → `deepseek-v4-flash`): `logger.error(str, { message: ... })` do `firebase-functions` sobrescreve `message` com um stack trace sintetico, escondendo o erro real — corrigido em `webhookHandler.ts` e `metaClient.ts` (mensagem real agora vai dentro da propria string, nao como campo `message` do objeto).
- **Sem impacto em `firestore.rules`** — `intent`/`suggestedScreen` nunca sao persistidos no Firestore, so usados pra decidir a resposta.
- Substitui/consolida as entradas de 2026-07-16 (`unsupported_action`), 2026-07-18 (`advisory_decision`) e 2026-07-22 (`bill_management_action`) abaixo — o raciocinio original de cada uma continua valido, so o mecanismo mudou (um intent nomeado por caso → um intent + tela sugerida).
- `npm --prefix functions run build`/`test` (124 testes, 3 novos em `messageFormat.test.ts` pra `outOfScopeMessage`) limpos. Deployado (`whatsappWebhook`, autorizado pelo dono) e testado ao vivo pelo WhatsApp real.
