...
| `functions/src/whatsapp/interpretMessage.ts` | Uma chamada DeepSeek (JSON mode) classifica a mensagem em `expense`/`income`/`transfer`/`card_purchase`/`create_category`/`question`/`out_of_scope`/`unclear` e ja extrai valor, descricao, categoria (a mais especifica entre as existentes), dados de categoria nova, conta(s) citada(s) (`accountId`, ou `sourceAccountId`/`destinationAccountId` pra transfer) e, quando `out_of_scope`, a tela sugerida do app (`suggestedScreen`: `transacoes`/`contas`/`contas_a_pagar`/`contas_a_receber`/`cartoes`/`metas`/`analise`/`assistente`/`geral`), tudo numa chamada so. Recebe a lista de contas ativas do workspace (id: nome) junto com a de categorias. Substitui o antigo `extractExpense.ts` (so cobria despesa). |
