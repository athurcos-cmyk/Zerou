...
- **Functions DEPLOYADAS** (Vic + alertas de orçamento), em duas etapas: o `--only functions` inicial
  derrubou 4 por quota de CPU do Cloud Run — erro de processo que o `docs/RUNBOOK.md` já mandava
  evitar desde 2026-07-21. Resolvido com o contorno documentado (lotes pequenos), sem aumento de
  quota. Nada quebrou: function que falha mantém a revisão anterior no ar.
