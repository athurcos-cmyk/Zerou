...
**Rebrand completo**: app renomeado de "Zerou" → "Granix" → **Granativa** (grana + ativa). Novo logo: duas bolas sobrepostas (sólida laranja + outline). Assets em `public/brand/granativa-*.png` (lowercase). Version strings internas do Firestore mantidas com prefixo `zerou-` para não invalidar registros existentes. LocalStorage keys (`zerou.themeMode`, `zerou.themeId`) também mantidas por ora.
